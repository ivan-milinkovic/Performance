...
// Collections always as values

final class JsonParserValues {
...
private class CollectionParser {
    ...
    private func popStack() {
        ...
        if var parent = stack.top() {
            parent.merge(current.value)
            stack.replaceTop(parent)
        } else {
            if case .map(let map) = current, map.isComplete == false {
...
private enum JsonCollection {
    ...
    mutating func merge(_ val: Any) {
        switch self {
        case .map(var map):
            map.consume(val)
            self = .map(map)
            
        case .array(var jsonArray):
...
private struct JsonMap {
    ...
    mutating func consume(_ val: Any) {
        if key == nil {
    ...
    mutating func consume(_ val: LiteralValue) {
        if key == nil {
